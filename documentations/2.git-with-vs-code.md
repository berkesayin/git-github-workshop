...
- Sol altta göründüğü gibi şu anda ``Feate branch'ı`` içerisinde bulunuyoruz. 
...
- Burada ``Git History`` üzerinden de commitlerimizi hangi branch üzerinde oluşturduğumuzu da görüntüleyebiliriz. Ayrıca orada gözüken Nav-Bar kısmında istediğimiz branch için oluşturulan commitleri de seçip görüntüleyebiliriz. 

- Şimdi de örneğin master branch'ine gidip bir değişiklik yapalım. Yani istediğimiz zaman master branch'e geçip orada commit'lere devam edebiliriz. İstediğimiz zaman da Feate branch içerisinde geliştirme yapabiliriz, ilerleyebiliriz. 

- Gördüğümüz gibi biz branchler arasında gezebiliyoruz, istediğimiz özelliği istediğimiz branch'te deneyebiliyoruz. Şimdi de merge özelliği ile çalıştığımız branch'i(Feate) ana projeye, ana branch'e(master) taşımayı görelim. 
...
- Burada merge işlemi sırasında eğer bir conflict yani çakışma olursa bizi uyarır, ve o conflict'i çözümlememiz gerekir. 

- Not: Merge işlemi de bir commit ile gerçekleşir. 

- Merge işleminden sonra eğer Feate branch'ini silmek istersek command pallette'de delete branch yazıp Feate branch'ını seçebiliriz, böylelikle Feate branch'ı silinmiş olur. 

- Ayrıca command pallette'de rename branch diyerek bir branch'ın ismini de değiştirebiliriz. Create Branch diyerek yeni bir branch oluşturabiliriz. Bu durumda o an hangi branch'te bulunuyorsak oradan bir dallanma yapısı oluşacaktır. Projenin o anki kopyasını alarak oradan(o anki versiyondan) ayrı bir branch çıkacaktır. 
 

## Git History İle Branch Oluşturma <a name="git-history-branch"></a>

- Bir önceki başlıkta şu anda hangi branch'teysek o branch'in son commit halinden yani son versiyonundan yeni branch oluşturmayı gördük. Peki ya daha eski bir commit'ten branch almak istersek yani projenin geçmişteki herhangi bir commit dosyasından branch almak istersek ne yapmamız gerekir? Bunun için `Git History'nin` commit geçmişlerini görünütlediğimiz ekranı kullanabiliriz.  

![pic](../images/27.1.git-history-branch.jpg)

- Burası Git History ile commit geçmişi listeleme ekranı. Burada geçmişte istediğimiz herhangi bir commit noktasından yani o anki versiyondan branch oluşturabiliriz. Bunun için de o commit'in en sağında bulunan tool'lar arasından "New Branch" == "Create Branch From Here" seçeneğini seçeriz. Orada yeni branch adını yazarak o commit noktasından yeni bir branch oluşturmuş oluruz. 

![pic](../images/27.2.git-history-branch.jpg)

- Merge işlemini yine aynı şekilde gerçekleştirebiliriz. 
...
- Şimdiye kadar yaptıklarımızı commit ile kaydettik. Burada dikkat etmemiz gereken bir nokta var. Şimdi dosyaları commit ettik fakat sonradan ignore etmemiz gereken bir dosya veya klasör olduğunu fark ettik. Daha önce commitlenmiş bir dosyayı `.gitignore` dosyasına yazdığımızda hala takip edildiğini görürüz. Çünkü daha önce commit'lenmiş dosyaların ignore edilmesi bu şekilde olmuyor, hala izlenmeye devam ediyor. Bunu engellemek için ufak bir console kodu da yazmamız gerekiyor. Bunun için de terminalde projemize locate olduktan sonra şu komutu yazmamız gerekli: 
`git rm --cached index.html`  : Burada cached'den sonra hangi dosyayı istiyorsak yolunu adını ve uzantısını doğru bir şekilde yazmamız gerekli. Bunu yazdıktan sonra artık bu dosyayı da normal bir şekilde .gitignore dosyasında yazarak git tarafından ignore edilmesini sağlayabiliriz. 

- Daha sonra eğer tekrar o dosyanın da takip edilmesini istersek o durumda git ignore dosyasından o dosyayı silebiliriz. 

- Özetle daha önce commitlenmiş bir dosyanın ignore edilmesi: 
1.`git rm --cached index.html`  &&  2.git ignore'a gidip dosya adının yazılması. 

- Not: Çalıştığımız platform Anroid, Visual Studio veya diğer platformlar olabilir. Bu gibi platformlarda projelerin ignore dosyaları büyük olabiliyor, çünkü bu editörler arkada çok fazla kod üretiyor ve ürettiği bu dosyalar çoğu zaman ihtiyacımız olmuyor. Bu dosyaların hepsini fark edip yapmak zor olabiliyor. Bunun için de github'da bir repo bulunuyor: 
`https://github.com/github/gitignore` 

- Burada kullandığımız dil veya editöre göre istediğimiz ignore dosyasını kopyalayıp kendi projemizde kullanabiliriz. 
